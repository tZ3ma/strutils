...
def sos_on(compound_string, using=str.capitalize, split_at='_', stitch_with='_'):
    """Split Operate Stitch. Split the paramref:`~sor_on.compound_string` at
    :paramref:`~sor_on.split_at` and operate on it with
    :paramref:`~sor_on.using` on each component to stitch it back
    together into one string using `:paramref:~sor_on.stitch_with`.
    ...
    Return
    ------
    sorred: :class:`~collections.abc.Generator`
...
def permute_splits(strings, stitch_with='_', split_at='_'):
    ...
    Return
    ------
    permmuted: :class:`~collections.abc.Generator`
...
def variate_compounds(compound_string, using=str.capitalize, permutate='True',
                      split_at='_', stitch_with='_'):
    """
    Vairate compound strings.
    ...
    if permutate:
        sorred = sos_on(compound_string, using=using, split_at=split_at,
                        stitch_with=split_at)
